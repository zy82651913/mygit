...
//拟合R平方
	public static double calRSquare_polyfit2(double x[], double[] y, double a,
			double b, double c, int cs) {

		int num = y.length;
		double[] yLine = new double[num];
		for (int i = 0; i < num; i++) {
//			yLine[i] = x[i] * b + a;
			yLine[i] = a + b * x[i] + c * Math.pow(x[i], cs);
		}

		return calRSquare(x, y, yLine, a, b);
	}

//	public static double calRSquare_linear(double x[], double[] y, double a,
//			double b) {
//
//		int num = y.length;
//		double[] yLine = new double[num];
...
//	}
//
//	public static double calRSquare_logest(double x[], double[] y, double a,
//			double b) {
//
//		int num = y.length;
//		double[] yLine = new double[num];
...
//	}
//
//	public static double calRSquare_powEST(double x[], double[] y, double c,
//			double b) {
//
//		int num = y.length;
//
...
//	}
//
//	public static double calRSquare_indexEST(double x[], double[] y, double c,
//			double m) {
//
//		int num = y.length;
//		double[] yLine = new double[num];
...
//	}
	...
	private static double calRSquare(double x[], double[] y, double[] yLine,
		...
		double sse = 0;
		for (int i = 0; i < num; i++) {
			sse += (y[i] - yLine[i]) * (y[i] - yLine[i]);
		}

		// 4、计算实际值减去平均值的平方和
		double sst = 0;
		for (int i = 0; i < num; i++) {
			sst += (y[i] - yAverage) * (y[i] - yAverage);
		}
